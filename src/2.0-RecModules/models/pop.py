...
class Pop(GeneralRecommender):
    ...
    def predict_for_graphs(self, interaction, user_count_start=None):
        item = interaction[self.ITEM_ID]

        flatten_item = item.view(-1)

        temp_item_cnt = torch.bincount(flatten_item)

        temp_max_cnt = torch.max(temp_item_cnt, dim=0)[0]

        result = torch.div(temp_item_cnt, temp_max_cnt, rounding_mode=None)

        final_result = result.squeeze(-1).repeat(item.shape[0], 1)
        return final_result

    def full_sort_predict(self, interaction):

        if self.max_cnt is None:
            self.calculate_loss(interaction)
