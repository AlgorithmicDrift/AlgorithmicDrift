...
class ItemEmbeddingsAdversarialTrainer(AbstractTrainer):
    ...
    def __init__(self, config, model, y, lambda_factor=20):
        super(ItemEmbeddingsAdversarialTrainer, self).__init__(config, model)
        ...
        self.adv = Classifier(
            n_features=config['embedding_size'],
            n_hidden=32,
            p_dropout=0.).to(
            config["device"])
        self.adv_criterion = nn.BCELoss()
        self.adv_optimizer = optim.Adam(self.adv.parameters(), lr=0.0005)

        self.y = y
        self.lambda_parameter = torch.cuda.FloatTensor([lambda_factor])
    ...
    def _build_optimizer(self, **kwargs):
        ...
        if self.config['reg_weight'] and weight_decay and weight_decay * \
                self.config['reg_weight'] > 0:
            self.logger.warning(
    ...
    def _train_epoch(
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            _, item_embeddings = self.model.forward()

            p_y = self.adv(item_embeddings)

            adv_loss = self.adv_criterion(p_y, self.y)
            adv_loss = adv_loss * -1
            ...
            fair_loss = loss + adv_loss * self.lambda_parameter

            self._check_nan(fair_loss)

            fair_loss.backward(retain_graph=True)

            total_loss = fair_loss.item() if total_loss is None else total_loss + losses.item()
            ...
            p_y = self.adv(item_embeddings.detach())
    ...
    def resume_checkpoint(self, resume_file):
        ...
        if checkpoint['config']['model'].lower(
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, losses):
        ...
        train_loss_output = (set_color('epoch %d training',
                                       'green') + ' [' + set_color('time',
                                                                   'blue') + ': %.2fs, ') % (epoch_idx,
                                                                                             e_time - s_time)
        if isinstance(losses, tuple):
    ...
    def _add_train_loss_to_tensorboard(
            self, epoch_idx, losses, tag='Loss/Train'):
        if isinstance(losses, tuple):
            for idx, loss in enumerate(losses):
    ...
    def _add_hparam_to_tensorboard(self, best_valid_result):
        ...
        self.tensorboard.add_hparams(
            hparam_dict, {
                'hparam/best_valid_result': best_valid_result})
    ...
    def fit(
        ...
        if self.config['train_neg_sample_args'].get(
                'dynamic', 'none') != 'none':
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            self.wandblogger.log_metrics(
                {'epoch': epoch_idx, 'train_loss': train_loss, 'train_step': epoch_idx}, head='train')
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_score_output = (set_color("epoch %d evaluating",
                                                'green') + " [" + set_color("time",
                                                                            'blue') + ": %.2fs, " + set_color(
                    "valid_score",
                    'blue') + ": %f]") % (epoch_idx,
                ...
                self.tensorboard.add_scalar(
                    'Vaild_score', valid_score, epoch_idx)
                self.wandblogger.log_metrics(
    ...
    def _spilt_predict(self, interaction, batch_size):
        ...
        for i in range(num_block):
            ...
            result = self.model.predict(
                Interaction(current_interaction).to(
                    self.device))
